...
bool AP_GPS_SBF::read(void)
{
    ...
    if (gps._auto_config != AP_GPS::GPS_AUTO_CONFIG_DISABLE)
    {
        if (config_step != Config_State::Complete)
        {
            ...
            else if (gps._raw_data == 2)
            { // only manage disarm/rearms when the user opts into it
                ...
                if (sbf_msg.data.bytes[0] == ':')
                {
                    ...
                    if (config_string != nullptr)
                    {
                                     ...
                                     sbf_msg.read - SBF_EXCESS_COMMAND_BYTES))
                        {
                            ...
                            case Config_State::SBAS:
                                _init_blob_index++;
                                if ((gps._sbas_mode == AP_GPS::SBAS_Mode::Disabled) || _init_blob_index >= ARRAY_SIZE(sbas_on_blob))
                                {
...
#pragma GCC diagnostic ignored "-Wfloat-equal" // suppress -Wfloat-equal as it's false positive when testing for DNU values
                if (((temp.Error & errorBits) == 0) && (temp.Mode > 0) && (temp.Heading != floatDNU))
                {
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wfloat-equal" // suppress -Wfloat-equal as it's false positive when testing for DNU values
                if (((temp.Error & errorBits) == 0) && (temp.Cov_HeadHead != floatDNU))
                {
#pragma GCC diagnostic pop
...
#pragma GCC diagnostic ignored "-Wfloat-equal" // suppress -Wfloat-equal as it's false positive when testing for DNU values
            ...
            if (temp.N == 0)
            { // no sub blocks so just bail, we can't do anything useful here
